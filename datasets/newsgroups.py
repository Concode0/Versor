...
class NewsgroupsCrossModalDataset(Dataset):
    """20 Newsgroups cross-modal: BERT vs TF-IDF.

    Modality A: BERT [CLS] → PCA → multivectors (multi-channel).
    Modality B: TF-IDF → TruncatedSVD → multivectors (multi-channel).
    Same documents, genuinely different representations.
    """
    ...
    def __init__(self, algebra, embedding_dim=6, in_channels=8, split='train',
        ...
        self.data_bert, self.data_tfidf, self.labels = self._load_or_compute()
    ...
    def _load_or_compute(self):
        bert_cache = self._cache_path('bert_cm')
        tfidf_cache = self._cache_path('tfidf_cm')
        labels_cache = self._cache_path('labels_cm')

        if (os.path.exists(bert_cache) and os.path.exists(tfidf_cache)
                and os.path.exists(labels_cache)):
            print(f">>> Loading cached embeddings from {self.cache_dir}")
            data_bert = torch.load(bert_cache, weights_only=True)
            data_tfidf = torch.load(tfidf_cache, weights_only=True)
            labels = torch.load(labels_cache, weights_only=True)
            return data_bert, data_tfidf, labels

        print(f">>> Computing BERT + TF-IDF embeddings for 20 Newsgroups ({self.split})...")
        print(f"    PCA/SVD: → {self.n_components} ({self.in_channels} channels × {self.embedding_dim} dims)")
        from sklearn.datasets import fetch_20newsgroups
        from transformers import BertTokenizer, BertModel
        ...
        tfidf_reduced = tfidf_reduced / (np.abs(tfidf_reduced).max() + 1e-6)

        # Embed into multivectors: each channel gets embedding_dim components
        vector_indices = [1 << i for i in range(self.embedding_dim)]

        data_bert = torch.zeros(len(targets), self.in_channels, self.algebra.dim)
        data_tfidf = torch.zeros(len(targets), self.in_channels, self.algebra.dim)

        for ch in range(self.in_channels):
            offset = ch * self.embedding_dim
            for i in range(self.embedding_dim):
                data_bert[:, ch, vector_indices[i]] = torch.tensor(
                    bert_reduced[:, offset + i], dtype=torch.float32
                )
                data_tfidf[:, ch, vector_indices[i]] = torch.tensor(
                    tfidf_reduced[:, offset + i], dtype=torch.float32
                )

        labels = torch.tensor(targets, dtype=torch.long)

        # Cache to disk
        torch.save(data_bert, bert_cache)
        torch.save(data_tfidf, tfidf_cache)
        torch.save(labels, labels_cache)
        print(f">>> Cached embeddings to {self.cache_dir}")
    ...
    def __getitem__(self, idx):
        return self.data_bert[idx], self.data_tfidf[idx], self.labels[idx]
