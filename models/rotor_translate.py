...
class RotorTranslator:
    ...
    def translate_direct(self, model, X_sample=None) -> List[RotorTerm]:
        ...
        Traces: embedding → GP activation → rotor sandwich → output head.
        Works per-channel, then combines via output_linear weights.
        ...
        # 2. For each block, apply activation + rotor sandwich
        for block_idx, block in enumerate(model.blocks):
            ...
            for c in range(C):
                ...
                # Apply GP activation if GeometricSquare
