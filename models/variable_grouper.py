...
class VariableGrouper:
    ...
    def _spectral_cluster(self, affinity, n_clusters):
        ...
        Args:
            affinity: [n, n] non-negative affinity matrix.
            n_clusters: Number of clusters.

        Returns:
            list[int]: Cluster labels for each variable.
        """
        n = affinity.shape[0]
        if n <= n_clusters:
            return list(range(n))
        ...
        # Eigenvectors of smallest eigenvalues
        try:
            eigenvalues, eigenvectors = np.linalg.eigh(L)
        except np.linalg.LinAlgError:
            return [i % n_clusters for i in range(n)]

        # Take first n_clusters eigenvectors (skip constant eigenvector at 0)
