...
class DoublePendulumODE:
    ...
    def energy_batch(self, states: np.ndarray) -> np.ndarray:
        ...
        t1 = states[..., 0]; t2 = states[..., 1]
        w1 = states[..., 2]; w2 = states[..., 3]
        l1, l2, m1, m2, g = self.l1, self.l2, self.m1, self.m2, self.g
    ...
    def generate_trajectories(self, x0_batch: np.ndarray,
                              n_steps: int, dt: float) -> np.ndarray:
        ...
        """
        traj = np.empty((n_steps, x0_batch.shape[0], x0_batch.shape[1]),
                        dtype=np.float64)
        traj[0] = x0_batch
        state = x0_batch.copy()
    ...
    def project_to_energy_shell(self, state_pred: np.ndarray,
                                state_input: np.ndarray) -> np.ndarray:
        ...
        t1 = state_pred[..., 0]; t2 = state_pred[..., 1]
        w1 = state_pred[..., 2]; w2 = state_pred[..., 3]
        l1, l2, m1, m2, g = self.l1, self.l2, self.m1, self.m2, self.g
        V = -(m1 + m2) * g * l1 * np.cos(t1) - m2 * g * l2 * np.cos(t2)
        T_pred = (0.5 * (m1 + m2) * l1 ** 2 * w1 ** 2
                  + 0.5 * m2 * l2 ** 2 * w2 ** 2
                  + m2 * l1 * l2 * w1 * w2 * np.cos(t1 - t2))
        T_target = E_target - V
...
class TrajectoryDataset(Dataset):
    ...
    def __init__(self, ode, n_traj: int = 500, traj_len: int = 200,
        ...
        x0_batch = np.stack(
            [ode.random_ic(rng, regime) for _ in range(n_traj)], axis=0,
        ).astype(np.float64)
        ...
        nexts = full[n_steps_ahead:n_steps_ahead + traj_len] \
            .reshape(-1, x0_batch.shape[1]).astype(np.float32)
...
class HamiltonianRotorNet(CliffordModule):
    ...
    def __init__(self, algebra: CliffordAlgebra, state_dim: int,
                 ...
                 coerce_even: bool = True, hermitian_renorm: bool = True):
    ...
    @torch.no_grad()
    def hidden(self, state: torch.Tensor, *,
               apply_coercion: bool = True) -> torch.Tensor:
...
@torch.no_grad()
def rollout(model: HamiltonianRotorNet, x0_norm: torch.Tensor, n_steps: int,
            *, ode=None, dataset: 'TrajectoryDataset' = None,
            energy_shell_project: bool = False) -> torch.Tensor:
    ...
    can_project = (
        energy_shell_project
        and ode is not None
        and dataset is not None
        and hasattr(ode, 'project_to_energy_shell')
    )
    for _ in range(n_steps - 1):
        ...
        if can_project:
            ...
            next_state = torch.as_tensor(
                dataset.normalize(nxt_phys), dtype=state.dtype, device=state.device,
            )
...
def _rotor_norm_residual(model: HamiltonianRotorNet) -> float:
    ...
    for module in model.modules():
        if isinstance(module, RotorLayer) and module.grade == 2:
            V = module._build_grade_element(
                model.even_grade_mask.device, model.even_grade_mask.dtype)
...
@torch.no_grad()
def _gauge_covariance_residual(model: HamiltonianRotorNet,
                               loader: DataLoader,
                               device: str) -> float:
...
def post_training_diagnostics(
    ...
    test_loader: DataLoader, device: str, *, rollout_steps: int,
    is_pendulum: bool, energy_shell_project: bool = False,
) -> Dict[str, float]:
    ...
    traj_norm = rollout(
        model, x0_t, rollout_steps,
        ode=ode, dataset=dataset,
    ...
    for states, _ in test_loader:
        raw_hiddens.append(
            model.hidden(states.to(device), apply_coercion=False))
        break
    ...
    diagnostics['gauge_covariance_residual'] = _gauge_covariance_residual(
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument('--coerce-even', dest='coerce_even', action='store_true',
                   default=True, help='(default for double_pendulum) Mask the '
                   'hidden state onto the even subalgebra between blocks.')
    p.add_argument('--no-coerce-even', dest='coerce_even', action='store_false')
    p.add_argument('--hermitian-renorm', dest='hermitian_renorm',
                   action='store_true', default=True,
                   help='Renormalize hidden multivectors to unit Hermitian '
                   'norm after each block.')
    p.add_argument('--no-hermitian-renorm', dest='hermitian_renorm',
                   action='store_false')
    p.add_argument('--energy-shell-project', dest='energy_shell_project',
                   action='store_true', default=True,
                   help='(pendulum only) Renormalize predicted (ω1, ω2) at '
                   'rollout so H(state_pred) = H(state_input).')
    p.add_argument('--no-energy-shell-project',
                   dest='energy_shell_project', action='store_false')
    return p.parse_args()
